...
cfg=load_config();t,y=load_trace();c,pub=load_published_calibration();cc=cfg["change_point"];pc=cfg["plateau"]
cps,score,idx=detect_change_points(t,y,cc["score_half_window_s"],cc["min_event_separation_s"],9,90,550,cc["peak_prominence_uA"])
pl=extract_plateaus(t,y,cps,pc["delay_s"],pc["duration_s"],pc["next_event_safety_s"])
rows=[]
...
plt.xlabel("Time (s)");plt.ylabel("Current (µA)");plt.title("Raw amperometric trace and extracted plateaus")
savefig(OUTPUT_FIGURES/"Figure2a_raw_trace_reconstruction.png")
plt.figure(figsize=(6,5));plt.scatter(pub,pl["mean"]);lims=[min(pub.min(),pl["mean"].min())-2,max(pub.max(),pl["mean"].max())+2]
plt.plot(lims,lims,linestyle="--");plt.xlabel("Published current (µA)");plt.ylabel("Extracted current (µA)")
